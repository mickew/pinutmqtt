...
internal class MqttClientService : IMqttClientService
{
    ...
    public async Task<bool> PublishStatusAsync(UPSStaus uPSStaus)
    {
        var payload = JsonSerializer.Serialize(uPSStaus);

        var result = await _mqttClient.PublishAsync(new MqttApplicationMessageBuilder()
            .WithTopic($"{_mqttSettings.Value.Topic}/{_mqttSettings.Value.ClientId}/status/ups")
            .WithPayload(payload)
            .Build());
        if (!result.IsSuccess)
        {
            _logger.LogWarning("Failed to publish ups status message to MQTT broker");
        }
    ...
    private async Task HandleConnectedAsync(MqttClientConnectedEventArgs args)
    {
        ...
        await AnnounceAsync(new AnnouncePayload(_mqttSettings.Value.ClientId!, "PINUT", "000000000000", _mqttSettings.Value.Server!));
